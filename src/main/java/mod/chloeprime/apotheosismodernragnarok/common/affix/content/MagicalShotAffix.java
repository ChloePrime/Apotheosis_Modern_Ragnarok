...
public class MagicalShotAffix extends DummySpecialAffix implements GunAffix {
    ...
    public void onBulletCreated(ItemStack gun, AffixInstance instance, BulletCreateEvent event) {
        event.getBullet().setData(ModContent.SinceMC1211.DataAttachments.IS_MAGICY, true);
    }

    @SubscribeEvent
    public static void preHurt(EntityHurtByGunEvent.Pre event) {
        if (!event.getBullet().getData(ModContent.SinceMC1211.DataAttachments.IS_MAGICY)) {
            return;
        }
    ...
    public static boolean clientIsMagicBullet(Projectile bullet) {
        return bullet.getData(ModContent.SinceMC1211.DataAttachments.IS_MAGICY);
    }
    ...
    public MagicalShotAffix(
            ...
            Map<LootCategory, Holder<SoundEvent>> sounds,
            Set<LootRarity> rarities) {
        super(def, categories, rarities);
        this.sounds = sounds;
    }
}
